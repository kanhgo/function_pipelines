## Aim: To extract calendar event information in a tabular format.

## STAGE 4
# Functions and pipelines

## Functions and pipeline for data extraction and transformation
...
### Pipeline - 1: Calendar data extraction, transformation and loading (local)
...
def batch_sum_v2(df, pause_duration=120, file_path='/filepath/OUTPUT/SUMMARIZED_DATA.xlsx'):
    ...
    for i in range(0, len(df), batch_size):
        ...
        for index, row in df.iloc[start_index:stop_index].iterrows():
            chunks = row['Chunks']
            summaries = [summarize_text_v2(chunk) for chunk in chunks if chunk]
...
### Pipeline - 2: Generation of summaries from extracted transcripts
...
## Functions and pipelines for mapping sub-types and skills


```python
# Requirements for get_subtype_v2
import pandas as pd
import numpy as np

from sklearn.metrics.pairwise import cosine_similarity   # A metric used to quantify similarity. Supports mapping by assessing similarity of topics and skill + goals
import tensorflow_hub as hub  # Provides access to the pre-trained models USE, which can be used without needing to train from scratch.
import tensorflow as tf  # For building and running machine learning models; imported to work with the model and handle the TensorFlow tensors that the model will produce.

# Requirements for get_skills_v2 and get_skills_links
from sentence_transformers import SentenceTransformer  # SentenceTransformer is a library that allows you to directly load the pre-trained model for sentence embedding
from sklearn.metrics.pairwise import cosine_similarity   # A metric used to quantify similarity. Supports mapping by assessing similarity of topics and skill + goals
...
def get_subtype_v2(df):
    '''
    
    Function created to predict or map a content piece or link to a sub-type. This is achieved by comparing the title and identified
    topics for the content pieces or links with the key words identified for each sub-type.

    Universal Sentence Encoder is used for sentence or longer text vectorization which produces a numeric representation of the semantics 
    or contextual meaning of each sentence or long form text. Cosine similarity is used to assess the similarity of compared sentences by 
    measuring the size of the angle between the associated vectors.

    N.B. IF THERE IS AN ERROR LOADING THE MODEL, REFER TO THE KAGGLE DOCUMENTATION AT "https://www.kaggle.com/models/google/universal-sentence-encoder
    /tensorFlow2/large" FOR GUIDANCE ON EMDEDDING.
    ...
    # Verify if required pre-trained USE model is loaded; load if necessary
    try:
        model_2 = hub.load("https://www.kaggle.com/models/google/universal-sentence-encoder/TensorFlow2/large/2")  #
    except Exception as e:
        print(f"Error loading model: {e}. Refer to https://www.kaggle.com/models/google/universal-sentence-encoder/tensorFlow2/large for guidance.")

    # GET SUB-TYPES
    # Define each sub-type - MINDSET
    MINDSET_rows = ['Building a global career in Barbados', 'Preparing yourself for remote work', 'Learning on the job', 'Collaborating with remote teammates', 'Landing the right job', 'Building your professional network', 'mindset', 'collaboration', 'learn', 'learn', 'feedback', 'job', 'portfolio', 'portfolios', 'networking', 'linkedin', 'conflict', 'skill']
    MINDSET = '. '.join(MINDSET_rows)
    MINDSET = MINDSET.lower()   # Make lower case
    
    # Define each sub-type - DATA
    DATA = "data, data analysis, data analyst, data science, data scientist, data engineering, data engineer, governance, analysis, data-driven, visualization, visualizations"

    # Define each sub-type - CUSTOMER SUCCESS
    CUST = "customer success, customers, customer, success, service, pain points, pain point, pain, relationship, relationships, troubleshoot, troubleshooting"

    # Define each sub-type - MARKETING
    MARK = "marketing, market, marketer, media, audience, brand, branding, social, campaign, campaigns, competitor, competitors"

    # Define each sub-type - DESIGN
    DESIGN = "design, designer, product, user, user experience, prototype, prototyping, wireframe, wireframes, mockup, mockups"

    # Define each sub-type - SALES
    SALES = "sales, sale, deal, leads, buyer, discovery"

    # "Define" terms that lack specific skills or goals
    SPECIAL_GEN = "specialization, specializations, specialisation, specialisations"
    
    PROJECTS = "project, projects, project management, management, scoping, scope, iteration, iterative, documentation"
     
    # Create a dataframe for subtypes
    df_subtypes = pd.DataFrame(columns=['Sub-Type','About', 'S_Embeddings'])

    # Populate the dataframe
    SUBTYPES = ['Mindset', 'Data', 'Customer Success', 'Marketing', 'Design', 'Sales', 'Specialization (General)', 'Projects']
    SUBTYPES_ABOUT = [MINDSET, DATA, CUST, MARK, DESIGN, SALES, SPECIAL_GEN, PROJECTS]
    for i in range(len(SUBTYPES)):
        df_subtypes.at[i,'Sub-Type']=SUBTYPES[i]
        df_subtypes.at[i,'About']=SUBTYPES_ABOUT[i]
    
    # Generate subtype embeddings (vectorize the sentences)
    for index, row in df_subtypes.iterrows():
        df_subtypes.at[index, 'S_Embeddings'] = model_2([row['About']])

    # To allow comparison of each row's topics with all of the different sub-types(at once), the mulitple rows of sub-type embeddings need to be stacked
    # into a single 2D array
    SUBTYPES_2D = np.vstack(df_subtypes['S_Embeddings'].values)
    
    # CLEAN AND PREPARE TOPICS FOR EMBEDDING
    # Replace /n with ''
    df_topics['Topics'] = df_topics['Topics'].str.replace('\n', ' ')

    # Remove rows with null topics
    nulls = df_topics[df_topics['Topics'].isna()].index.tolist()
    df_topics.drop(index=nulls, axis=0, inplace=True)

    # Add title to the topics to aid with correct mapping (particularly useful for specializations)
    df_topics['Topics_plus'] = df_topics['Title'] + '. ' + df_topics['Topics']

    # Make Topics_plus lower case (testing to see if it matters)
    df_topics['Topics_plus'] = df_topics['Topics_plus'].str.lower()
    
    # Add an empty column for embeddings and one for Auto Sub-Type
    df_topics['S_Embeddings'] = ''
    df_topics['Auto Sub-Type'] = ''
    # df_topics will be the table exported or returned for passing to the next function in the mapping pipeline

    # Generate Embeddings for Topics_plus (vectorize the sentences)
    for index, row in df_topics.iterrows():
        df_topics.at[index, 'S_Embeddings'] = model_2([row['Topics_plus']])
        
    # SIMILARITY ASSESSMENT
    # Evaluate the similarity of each row's topic against the subtypes and map to content
    for index, row in df_topics.iterrows():
        similarity_matrix = cosine_similarity(row['S_Embeddings'], SUBTYPES_2D)    # Evaluating the similarity of each row's topic against all subtypes
        similarity_df = pd.DataFrame(similarity_matrix, columns=[x+1 for x in range(len(SUBTYPES))])
        mapped_subtypes_ID = similarity_df.iloc[0].idxmax()  # Get column ID of max similarity score for each row    
        
        df_topics.at[index,'Auto Sub-Type'] = df_subtypes.loc[mapped_subtypes_ID-1, 'Sub-Type']
...
def get_skills_v3(df, category, df_skills='/filepath/WORKSPACE/Skills.csv'):
    ...
    Mindset and specialisation content pieces will be considered separately. Assessment of project items will be done manaully.
    ...
    # Get topics
    if category == "M":
        df_temp = df[df["Auto Sub-Type"] == "Mindset"]
        df_topics = df_temp[['ID', 'Topics', 'Auto Sub-Type']].copy()
    elif category == "S":
        #df_temp = df.query("'Auto Sub-Type' in ['Data', 'Customer Success', 'Design', 'Sales', 'Marketing', 'Specialization (General)']")
        df_temp = df[df["Auto Sub-Type"].isin(["Data", "Customer Success", "Design", "Sales", "Marketing", "Specialization (General)"])]
        df_topics = df_temp[['ID', 'Topics', 'Auto Sub-Type']].copy()
    ...
    df_topics['Topics'] = df_topics['Topics'].str.split('.')  #\n was removed in the preceding get_sub-types function
    ...
    # Dropm skills without levels
    nulls = df_skills[df_skills['Level'].isna()].index.tolist()
    df_skills.drop(index=nulls, axis=0, inplace=True)
    ...
    mapped_df = pd.DataFrame(columns=['ID', 'Topics', 'Auto Sub-Type', 'Auto Related Skill', 'Auto Level'])
    ...
    for index, row in df_topics.iterrows():
        ...
        if topics_len > 0:          # Handling the ocurrence of empty topic cells
            ...
            for i in range(topics_2D.shape[0]):     # shape[0] calls the number of rows and hence the number of iterations of the for loop required
                ...
                if max(similarity_df.iloc[0]) >= 0.3:    # setting a threshold or min value of 0.3 (tried from none to 0.45)
                    s_ID = similarity_df.iloc[0].idxmax()
                    if df_topics.loc[index, 'Auto Sub-Type'] == df_skills.loc[s_ID-1,'Sub-Type']:  # Check if the auto-skill suggested is related to the previously determined auto sub-type
        ...
        if len(mapped_skill_ID) > 0:
            ...
            mapped_df.at[index, 'Auto Sub-Type'] = row['Auto Sub-Type']
            mapped_df.at[index, 'Auto Related Skill'] = list(set(mapped_skills))     # remove duplicates
            mapped_df.at[index, 'Auto Level'] = [f'L{df_skills.loc[(a-1),'Level']} - {mapped_skills[mapped_skill_ID.index(a)]}' for a in mapped_skill_ID]
...
def get_skills_links_v2(df, category, df_skills='/filepath/WORKSPACE/Skills.csv'):
    ...
    if category == "M":
        df_temp = df[df["Auto Sub-Type"] == "Mindset"]
        df_topics = df_temp[['ID_Links', 'Topics', 'Auto Sub-Type']].copy()
    elif category == "S":
        # df_temp = df.query("'Auto Sub-Type' in ['Data', 'Customer Success', 'Design', 'Sales', 'Marketing', 'Specialization (General)']")
        df_temp = df[df["Auto Sub-Type"].isin(["Data", "Customer Success", "Design", "Sales", "Marketing", "Specialization (General)"])]
        df_topics = df_temp[['ID_Links', 'Topics', 'Auto Sub-Type']].copy()
    ...
    # Create the dataframe that will hold the mapped skills (created outside of the "for loop" to ensure it is not reset with each loop)
    mapped_df = pd.DataFrame(columns=['ID_Links', 'Topics', 'Auto Sub-Type', 'Auto Related Skill', 'Auto Level'])
